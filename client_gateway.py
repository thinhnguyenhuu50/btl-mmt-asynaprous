...
def background_tasks():
    """Radar chạy ngầm: Học danh bạ và Khám sức khỏe Proxy liên tục"""
    ...
    while True:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=1.0) as res:
                ...
                if data.get("peers"): 
                    with _lock: CACHED_BACKENDS = sorted(data["peers"]) # Sắp xếp để chia tải chuẩn
        except: pass
        ...
        try:
            ...
            s.close()
            if not PROXY_ALIVE:
                print("✅ Proxy 81 đã sống lại! Trả quyền điều phối cho Proxy.")
            PROXY_ALIVE = True
        except:
            if PROXY_ALIVE:
                print("⚠️ Proxy 81 mất tín hiệu! Kích hoạt chế độ sinh tồn.")
            PROXY_ALIVE = False
...
def fix_headers(request_bytes, response_bytes):
    try:
        ...
        elif '/login' in path or '/channels' in path or '/messages' in path or '/broadcast-peer' in path:
...
def handle_browser_request(client_sock):
    ...
    try:
        ...
        if PROXY_ALIVE:
            # Nếu Radar báo Proxy sống -> Gửi lên Proxy
            try:
                raw_response = forward_to(81, request_bytes)
                FALLBACK_NODE = None 
            except Exception as e:
                # TUYỆT ĐỐI KHÔNG RETRY! Nếu đứt mạng giữa chừng, bỏ qua để tránh nhân bản 2 tin nhắn.
                pass 
        else:
            # Nếu Radar báo Proxy chết -> Bám chặt vào Node dự phòng
            with _lock:
...
def run_gateway(my_port):
    ...
    server.listen(100)
    print(f"🖥️ Client Gateway đang mở tại http://127.0.0.1:{my_port}")
